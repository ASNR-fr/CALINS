...
The Generalized Linear Least Squares Method (GLLSM) provides a rigorous statistical framework for the consistent adjustment of nuclear data and the estimation of associated biases and uncertainties based on integral experiments. The method is rooted in a Bayesian formulation of the adjustment problem, where prior knowledge on nuclear data and experimental information are combined in a unified least-squares minimization.

At its core, GLLSM is derived from the minimization of a global discrepancy function that measures the weighted misfit between calculated and measured responses, while explicitly accounting for the uncertainties and correlations associated with both nuclear data and experiments. This discrepancy is typically expressed in terms of a quadratic form involving the differences between experimental and calculated observables, propagated through sensitivity coefficients with respect to nuclear data parameters. The weighting is provided by the covariance matrices of nuclear data and experimental uncertainties, ensuring that each contribution is treated in a statistically consistent manner.

Within this framework, the adjustment of nuclear data is obtained by minimizing this global objective function, which leads to a linear system of equations linking the posterior corrections on nuclear data to the sensitivity profiles of the considered integral experiments. The resulting solution provides not only updated (posterior) nuclear data estimates, but also a quantified reduction of uncertainties and a consistent propagation of correlations between nuclear data and integral observables.

An important feature of GLLSM is that it explicitly separates prior knowledge from information gained through integral experiments. This allows the method to produce a posterior bias estimate for application systems, along with a rigorous uncertainty evaluation that reflects both experimental constraints and nuclear data covariances. As a result, GLLSM is widely used in nuclear criticality safety studies to derive application-specific bias estimates and Upper Subcritical Limits (USLs), while maintaining strict consistency with the underlying nuclear data uncertainty structure.
