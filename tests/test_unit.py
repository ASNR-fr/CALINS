...
sensi_proper_path_1 = os.path.join(TEST_INPUTS_ROOT, "sensi_proper_1.sdf")
sensi_proper_path_2 = os.path.join(TEST_INPUTS_ROOT, "sensi_proper_2.sdf")
sensi_proper_path_3 = os.path.join(TEST_INPUTS_ROOT, "sensi_proper_3.sdf")
sensi_proper_path_4 = os.path.join(TEST_INPUTS_ROOT, "sensi_proper_4.sdf")

sensi_case_1 = cl.Case(sdf_path=sensi_proper_path_1)
sensi_case_2 = cl.Case(sdf_path=sensi_proper_path_2)

cov_scale_proper_path_1 = os.path.join(TEST_INPUTS_ROOT, "cov_proper_1.txt")
cov_dataf_1_scale_tuple = cl.format_scale_txt_to_dataframe(cov_scale_proper_path_1)
cov_dataf_1_scale = cov_dataf_1_scale_tuple[0]  # Extract DataFrame from tuple
cov_1_nd = cl.NDCovariances(cov_scale_proper_path_1, format="coverx_text")
...
class TestFunctions(unittest.TestCase):
    ...
    def test_formatage(self):
        ...
        # --- Test format_scale_txt_to_dataframe with empty file
        try:
            cl.format_scale_txt_to_dataframe(os.path.join(TEST_INPUTS_ROOT, "empty_file.txt"))
        except IndexError as e:
            print("Test successfull empty file :", str(e))

        # --- Test formatting cov data - SCALE
        if len(cov_dataf_1_scale_tuple[0]["STD"]) != 0:
            print("Test successfull for reading covariances file (type COVERX txt)")

        # --- Test formatting cov data - COMAC
        cov_comac_proper_path_2 = os.path.join(TEST_INPUTS_ROOT, "cov_proper_2_comac")
        cov_dataf_2_comac = cl.format_comac_to_dataframe(cov_comac_proper_path_2)
        print("Test successfull for reading covariances file (type COMAC)")

        # --- Test NDCovariances object creation
        cov_obj_scale = cl.NDCovariances(input_path=cov_scale_proper_path_1, format="coverx_text")
        ...
        try:
            cov_comac_corrupted_path_1 = os.path.join(TEST_INPUTS_ROOT, "cov_corrupted_2_comac")
        ...
        # --- Test SDF formatting with addition of sensitivities for equal isotope and reaction numbers (typically H1 and H1-H2O)
        sensi_df = cl.format_sensi_to_dataframe(input_sdf_path=sensi_proper_path_3, std=True)
        ...
        # --- Test condense sdf
        input_path = os.path.join(TEST_INPUTS_ROOT, "sensi_proper_1.sdf")
        output_path = os.path.join(TEST_INPUTS_ROOT, "sensi_proper_1_condensed.sdf")
    ...
    def test_E(self):
        ...
        E = cl.calcul_E(case_1=sensi_proper_path_1, case_2=sensi_proper_path_2)
        self.assertAlmostEqual(E, 0.7182, places=4)

        E = cl.calcul_E(case_1=sensi_case_1, case_2=sensi_case_2)
        self.assertAlmostEqual(E, 0.7182, places=4)

        E = cl.calcul_E(case_1=sensi_proper_path_1, case_2=sensi_case_2)
        self.assertAlmostEqual(E, 0.7182, places=4)
    ...
    def test_SSR(self):
        ...
        SSR = cl.calcul_SSR(appl_case=sensi_proper_path_1, bench_case=sensi_proper_path_2)
        self.assertAlmostEqual(SSR, 0.5714, places=4)

        SSR = cl.calcul_SSR(appl_case=sensi_case_1, bench_case=sensi_case_2)
        self.assertAlmostEqual(SSR, 0.5714, places=4)

        SSR = cl.calcul_SSR(appl_case=sensi_proper_path_1, bench_case=sensi_case_2)
        self.assertAlmostEqual(SSR, 0.5714, places=4)
        print("Test successfull for calculation type SS")

    def test_G_CEA(self):

        G = cl.calcul_G_CEA(appl_case=sensi_proper_path_1, bench_case=sensi_proper_path_2)
        self.assertAlmostEqual(G, 0.5714, places=4)

        G = cl.calcul_G_CEA(appl_case=sensi_case_1, bench_case=sensi_case_2)
        self.assertAlmostEqual(G, 0.5714, places=4)

        G = cl.calcul_G_CEA(appl_case=sensi_proper_path_1, bench_case=sensi_case_2)
        self.assertAlmostEqual(G, 0.5714, places=4)
    ...
    def test_G(self):
        ...
        G = cl.calcul_G(case_1=sensi_proper_path_1, case_2=sensi_proper_path_2)
        self.assertAlmostEqual(G, 0.71795, places=4)

        G = cl.calcul_G(case_1=sensi_case_1, case_2=sensi_case_2)
        self.assertAlmostEqual(G, 0.71795, places=4)

        G = cl.calcul_G(case_1=sensi_proper_path_1, case_2=sensi_case_2)
        self.assertAlmostEqual(G, 0.71795, places=4)
    ...
    def test_Ck(self):
        ...
        Ck = cl.calcul_Ck(case_1=sensi_proper_path_1, case_2=sensi_proper_path_2, cov_data=cov_1_nd)
        self.assertAlmostEqual(Ck, 0.7807, places=4)

        Ck = cl.calcul_Ck(case_1=sensi_case_1, case_2=sensi_case_2, cov_data=cov_1_nd)
        self.assertAlmostEqual(Ck, 0.7807, places=4)

        Ck = cl.calcul_Ck(case_1=sensi_case_1, case_2=sensi_proper_path_2, cov_data=cov_1_nd)
        self.assertAlmostEqual(Ck, 0.7807, places=4)

        # Test with NDCovariances object
        cov_obj = cl.NDCovariances(input_path=cov_scale_proper_path_1, format="coverx_text")
    ...
    def test_prior_unc(self):
        ...
        prior_unc = cl.calcul_uncertainty(appl_case=sensi_proper_path_1, cov_data=cov_1_nd)
        self.assertAlmostEqual(prior_unc.value, 1.12811, places=4)
        ...
        prior_unc = cl.calcul_uncertainty(appl_case=sensi_proper_path_3, cov_data=cov_1_nd)
        self.assertAlmostEqual(prior_unc.value, 1.12811, places=4)

        cl.calcul_uncertainty(appl_case=sensi_proper_path_3, cov_data=cov_1_nd)
    ...
    def test_assimilation(self):
        # --- Test wrong isotope list as input
        try:
            assim = cl.Assimilation(
                benchmarks_list=[sensi_proper_path_1],
                appl_case=sensi_proper_path_2,
                cov_data=cov_1_nd,
        ...
        assim = cl.Assimilation(
            benchmarks_list=[sensi_proper_path_2, sensi_proper_path_4],
        ...
        # --- Test calculation with NDCovariances object (recommended default option)
    ...
    def test_USL_gllsm(self):
        ...
        # Check USL value (placeholder - to be replaced with reference value)
    ...
    def test_USL_parametric_normality_test(self):
        ...
        assim = cl.Assimilation(
            benchmarks_list=[sensi_proper_path_2, sensi_proper_path_4, sensi_proper_path_3],
            appl_case=sensi_proper_path_1,
            cov_data=cov_1_nd,
        )
    ...
    def test_USL_nonparametric_sufficient_benchmarks(self):
        ...
        bench_list = [sensi_proper_path_2, sensi_proper_path_3, sensi_proper_path_4] * 5  # 15 benchmarks
        assim = cl.Assimilation(
